...
\def\mybooktitlepreamble{% before each new book title, we emit this:
    ...
    \noindent%
    \makebox[0pt][l]{\noindent\raisebox{0.0\baselineskip}[0pt]{%
        \noindent\color{mygray}\rule{\linewidth - 1pt}{0.2pt}}}%
}
...
\newcommand\mybooktitlebox[1]{%
    ...
    \null\hfill%center it
        ...
        \raisebox{0.9\baselineskip}[0.5\baselineskip][0pt]{%
...
\def\mychapterfontname{\mysansfontname}% verse number font scaling
...
\def\mybookspace{7.0\baselineskip}% needspace for chapter title (should be 2\baselineskip - but in multicolumn, it doesn't work)
\def\mychapterraise{1pt}% push the chapter number up this much
\def\myverseraise{2pt}% verse number superscript scaling
\def\myversehspace{1pt plus 1pt}% space after verse number
...
\def\myversefontname{\mysansfontname}
\def\mysansfontiiname{\mysansfontname}
...
\def\myhypenpenalty{130}%   Lower penalty means more hypenation and fewer pages
\def\showredgrid{%
    \usepackage{tikz}
    \usepackage{eso-pic}
    \AddToShipoutPicture{%
...
\begin{afrikaans}%
\hyphenation{teë-beeld Jesus Christus berugte honderd-voudig Judas}%
\end{afrikaans}%
%%
...
\newcommand{\PPversei}[4]{\PPverseref{#2}\ppsaveposition{#3}\pplang{#4}\ppsavepositionk{#3}}% the space after the verse is a thing
\newcommand{\PPverseii}[4]{%
  ...
  %\else
    \PPsyntheticparii% QK%
  %\fi
\raisebox{\myverseraise}[0pt]{\myversefont\textbf{#1}}\ppsaveposition{#3}{\versehskip\PPverseref{#2}\pplang{#4}\ppsavepositionk{#3}}}% the space after the verse is a thing
\newcommand{\PPverse}[4]{\par\raisebox{\myverseraise}[0pt]{\myversefont\textbf{#1}}\ppsaveposition{#3}{\versehskip\PPverseref{#2}\pplang{#4}\ppsavepositionk{#3}}}% the space after the verse is a thing
\newcommand{\PPspace}{\par\null}% the space after the verse is a thing
...
%\newcommand{\PPpsalmauthor}[1]{{\ppfontadjust\pplang{\em #1}}\PPsyntheticparii}
...
\begin{textblock}{1}(#1,#2)%
    %\rule[0em]{1em}{1ex}%
    \hfuzz=72pt
    \raisebox{0pt}[0pt]{\hspace{#3}\color{#4}\rule[0em]{13mm}{#5}}% vert offset, width, height
\end{textblock}
}
...
\newcommand{\PPbookmarker}[1]{\PPthumbmark{0}{#1}{-10mm}{lightgray}{2pt}}%
